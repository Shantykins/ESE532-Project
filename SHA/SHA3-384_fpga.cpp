...
void SHA_new(char* data, uint64_t len, char* digest)
{
    /* empty message with padding */
    uint64_t bit_len = len * 8;
    unsigned char len_bytes[8];
    len_bytes[0] = (bit_len >> 56) & 0xFF;
    len_bytes[1] = (bit_len >> 48) & 0xFF;
    len_bytes[2] = (bit_len >> 40) & 0xFF;
    len_bytes[3] = (bit_len >> 32) & 0xFF;
    len_bytes[4] = (bit_len >> 24) & 0xFF;
    len_bytes[5] = (bit_len >> 16) & 0xFF;
    len_bytes[6] = (bit_len >> 8) & 0xFF;
    len_bytes[7] = (bit_len >> 0) & 0xFF;

    int buff_len = len+9;
    //if modvar  == 0
    // if modvar > 9 bytes
    // if modvar < 9 bytes
    int pad = (buff_len+63)/64;

    int zeros = 64*pad-buff_len;
    int message_len = 64*pad;
    ...
    for(int j = len+zeros + 1;j<message_len;j++)
    {
        message[j] = len_bytes[j - len - zeros - 1];
    }
